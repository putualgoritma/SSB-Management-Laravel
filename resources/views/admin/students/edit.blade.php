    ...
    <div class="card-body">
        <form action="{{ route("admin.students.update", [$student->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('grade_id') ? 'has-error' : '' }}">
                <label for="gender">{{ trans('global.student.fields.grade_id') }}*</label>
            ...
            <div class="form-group {{ $errors->has('team_id') ? 'has-error' : '' }}">
                <label for="gender">{{ trans('global.student.fields.team_id') }}*</label>
                <select id="team_id" name="team_id" class="form-control" value="{{ old('team_id', isset($student) ? $student->team_id : '') }}">
                    @foreach($teams as $team)
                    <option value="{{$team->id}}" {{$student->team->id == $team->id ? 'selected' : ''}}>{{ $team->name}}</option>
                    @endforeach
                    </select>
            ...
            <!-- <div class="form-group {{ $errors->has('roles') ? 'has-error' : '' }}">
                ...
                <select name="grade_id" id="grade_id" class="form-control select2" multiple="multiple" required>
                    @foreach($grades as $grade)
                    <option value="{{$grade->id}}" {{$student->grade->id == $grade->id ? 'selected' : ''}}>{{ $grade->name}}</option>
                    @endforeach
                </select>
